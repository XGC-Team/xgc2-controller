...
#include "multirotor_reference_trajectory/core/minco_backend/flatness.hpp"
#include "multirotor_reference_trajectory/core/minco_backend/lbfgs.hpp"
#include "multirotor_reference_trajectory/core/minco_backend/minco.hpp"
...
uint32_t limitFlags(const FlatOutput& output, const TrajectoryLimits& limits) {
    ...
    const auto full = FlatnessMapper(9.8066, limits.min_specific_thrust).map(output);
...
bool smoothedL1(double x, double mu, double& value, double& derivative) {
    if (x < 0.0) {
        value = 0.0;
        derivative = 0.0;
        return false;
...
void forwardT(const Eigen::VectorXd& tau, Eigen::VectorXd& times) {
...
void backwardT(const Eigen::VectorXd& times, Eigen::VectorXd& tau) {
...
void backwardGradT(const Eigen::VectorXd& tau, const Eigen::VectorXd& grad_times,
...
int spatialDim(const WaypointConstraint& constraint) {
    return constraint.type == WaypointConstraintType::kPoint ? 0 : 3;
}

double sphereRadius(const WaypointConstraint& constraint) {
...
Eigen::Vector3d halfExtents(const WaypointConstraint& constraint) {
...
Eigen::Matrix3d rotationOf(const WaypointConstraint& constraint) {
...
Eigen::Vector3d mapConstraintPoint(const WaypointConstraint& constraint,
                                   const Eigen::Vector3d& xi) {
    if (constraint.type == WaypointConstraintType::kPoint) {
        return constraint.position;
    }
    if (constraint.type == WaypointConstraintType::kSphere) {
    ...
    if (constraint.type == WaypointConstraintType::kGate && half.x() <= 0.0) {
        half.x() = 0.0;
    }
...
Eigen::Vector3d constraintGradXi(const WaypointConstraint& constraint, const Eigen::Vector3d& xi,
...
void dynamicPenalty(const Eigen::VectorXd& times, const Eigen::MatrixX3d& coeffs,
                    const WaypointProblem& problem, double& cost, Eigen::VectorXd& grad_times,
...
class MincoWaypointOptimizer {
   public:
    explicit MincoWaypointOptimizer(const WaypointProblem& problem) : problem_(problem) {
    ...
    bool optimize(PiecewisePolynomialEvaluator& evaluator, uint32_t& flags) {
        if (piece_count_ <= 0) {
        ...
        double min_cost = 0.0;
        const int ret = lbfgs::lbfgs_optimize(x, min_cost, &MincoWaypointOptimizer::costFunction,
...
AnalyticEvaluator::AnalyticEvaluator(AnalyticParameters params) : params_(std::move(params)) {
    ...
    if (!finiteScalar(params_.z_frequency) || params_.z_frequency <= 0.0) {
...
bool PiecewisePolynomialEvaluator::setSegments(std::vector<PolynomialSegment> segments,
    ...
    for (const auto& segment : segments_) {
        if (!finiteScalar(segment.duration) || segment.duration <= 0.0 ||
...
bool MincoWaypointSolver::solve(const WaypointProblem& problem,
    ...
    for (const auto& constraint : problem.constraints) {
        if (!finiteVector(constraint.position) || !finiteVector(constraint.size) ||
            !finiteScalar(constraint.orientation.norm()) ||
            constraint.orientation.norm() < kMinNorm) {
            local_flags |= kFlagInvalidInput;
        }
    }
    ...
    for (const double duration : problem.segment_times) {
        if (!finiteScalar(duration) || duration <= kMinDuration) {
            local_flags |= kFlagInvalidInput;
        }
    }
    if (!finiteVector(problem.start_velocity) || !finiteVector(problem.start_acceleration) ||
        !finiteVector(problem.end_velocity) || !finiteVector(problem.end_acceleration) ||
        !finiteScalar(problem.desired_speed) || !finiteScalar(problem.time_weight) ||
        !finiteScalar(problem.dynamic_penalty_weight) || !finiteScalar(problem.rel_cost_tol)) {
        local_flags |= kFlagInvalidInput;
    }
    ...
    MincoWaypointOptimizer optimizer(problem);
...
bool SampledEvaluator::setSamples(std::vector<SampledPoint> samples) {
    ...
    for (const auto& sample : samples_) {
        if (!finiteScalar(sample.t) || sample.t < last_t ||
            !TrajectoryValidator::finite(sample.flat)) {
            flags_ |= kFlagInvalidInput;
            return false;
        }
        last_t = sample.t;
...
FullStateReference FlatnessMapper::map(const FlatOutput& flat) const {
    ...
    if (!finiteScalar(y_raw.norm()) || y_raw.norm() < 1.0e-7) {
    ...
    if (!finiteVector(output.body_rate) || !finiteVector(output.angular_acceleration) ||
        !finiteScalar(output.attitude.norm())) {
        output.flags |= kFlagNonFinite;
    }
    return output;
}
