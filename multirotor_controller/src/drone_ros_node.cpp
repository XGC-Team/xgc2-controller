#include "multirotor_controller/drone_ros_node.h"
#include "multirotor_controller/output/control_output_consumer.h"
#include "multirotor_controller/output/debug_output_consumer.h"
#include "multirotor_controller/output/nmpc_output_consumer.h"
#include "multirotor_controller/output/reference_activation_output_consumer.h"
#include "xgc2_geometry/geometry/math_helpers.h"
...
std::string resolveTopicName(const ros::NodeHandle &nh,
...
DroneRosNode::DroneRosNode(ros::NodeHandle &nh)
    : nh_(nh), nh_private_("~"), sensor_data_(),
  ...
  auto px4_consumer =
      std::make_unique<Px4ServiceOutputConsumer>(nh_, output_event_executor_);
  ...
  output_event_dispatcher_.addConsumer(std::make_unique<DebugOutputConsumer>(
      nh_, output_event_executor_, controller_, sensor_data_,
      vrpn_quality_stats_, debug_print_, kRosQueueSize));
  output_event_dispatcher_.addConsumer(
      std::make_unique<ReferenceActivationOutputConsumer>(
          nh_, output_event_executor_, controller_, kRosQueueSize));
  ...
  sensor_input_producer_ = std::make_unique<SensorInputProducer>(
      nh_, sensor_data_, vrpn_quality_stats_, kRosQueueSize, post_input_event,
      [this] {
  ...
  command_input_producer_ = std::make_unique<CommandInputProducer>(
      nh_, post_input_event, kRosQueueSize);
  trajectory_input_producer_ = std::make_unique<TrajectoryInputProducer>(
      nh_, sensor_data_, controller_.referenceTrajectoryBuffer(),
  ...
  ROS_INFO("  - alg/reference_trajectory/activate");
}
...
void DroneRosNode::loadControllerConfig() {
  ...
  ros1_utils::getParamWithLog(nh_private_, "takeoff_altitude",
                              config.takeoff_altitude, "Takeoff altitude (m)");
  const std::string uav_name =
      ros1_utils::currentNameFromNamespacePrefix("/uav");
  double per_uav_takeoff_altitude = config.takeoff_altitude;
  if (!uav_name.empty() && nh_private_.getParam("takeoff_altitudes/" + uav_name,
                                                per_uav_takeoff_altitude)) {
    if (std::isfinite(per_uav_takeoff_altitude) &&
        per_uav_takeoff_altitude > 0.0) {
      config.takeoff_altitude = per_uav_takeoff_altitude;
      ROS_INFO("[DroneRosNode] Per-UAV takeoff altitude for %s: %.3f m",
               uav_name.c_str(), config.takeoff_altitude);
    } else {
      ROS_WARN("[DroneRosNode] Invalid per-UAV takeoff altitude for %s: %.3f, "
               "keeping %.3f m",
               uav_name.c_str(), per_uav_takeoff_altitude,
  ...
  config.planning_period = nh_private_.param(
      "planning_period",
      nh_private_.param("sampling_time", config.planning_period));
  if (!std::isfinite(config.planning_period) || config.planning_period <= 0.0) {
    ROS_WARN(
        "[DroneRosNode] Invalid planning_period/sampling_time; using 0.100 s");
    config.planning_period = 0.1;
  }
  ROS_INFO("[DroneRosNode] MPC planning period: %.3f s",
           config.planning_period);

  // ========== MPC轨迹跟踪控制模式 ==========
  // 读取控制模式 (0=PX4_CASCADE_PID, 1=PURE_SLIDING_MODE, 2=HYBRID_CONTROL)
  int control_mode_int = 0;
  ros1_utils::getParamWithLog(nh_private_, "control_mode", control_mode_int,
                              "Control mode");

  // 转换为枚举类型（防御性检查：确保值在有效范围内）
  if (control_mode_int < 0 || control_mode_int > 2) {
    ROS_WARN(
        "[DroneRosNode] Invalid control_mode=%d, using default PX4_CASCADE_PID",
        control_mode_int);
    control_mode_int = 0;
  }
  ...
  } else if (tracking_backend == "nmpc_attitude_rate" ||
             tracking_backend == "nmpc") {
    config.tracking_backend = TrackingBackend::NMPC_ATTITUDE_RATE;
  } else {
    ROS_WARN(
        "[DroneRosNode] Unknown tracking_backend=%s, using legacy_mpc_lifter",
  ...
  // ========== 偏航角控制开关 ==========
  ros1_utils::getParamWithLog(nh_private_, "enable_yaw_control",
                              config.enable_yaw_control, "Enable yaw control");
  ...
  nh_private_.param("nmpc/log_period", config.nmpc.log_period,
                    config.nmpc.log_period);

  nh_private_.param("hover_thrust/enabled", config.nmpc.hover_thrust_enabled,
                    config.nmpc.hover_thrust_enabled);
  nh_private_.param("nmpc/hover_thrust_enabled",
                    config.nmpc.hover_thrust_enabled,
                    config.nmpc.hover_thrust_enabled);
  nh_private_.param("hover_thrust/timeout", config.nmpc.hover_thrust_timeout,
                    config.nmpc.hover_thrust_timeout);
  nh_private_.param("nmpc/hover_thrust_timeout",
                    config.nmpc.hover_thrust_timeout,
                    config.nmpc.hover_thrust_timeout);
  nh_private_.param("nmpc/solve_timeout", config.nmpc.solve_timeout,
                    config.nmpc.solve_timeout);
  ...
  nh_private_.param("nmpc/reference_sample_dt",
                    config.nmpc.reference_sample_dt,
                    config.nmpc.reference_sample_dt);

  if (!std::isfinite(config.nmpc.control_period) ||
      config.nmpc.control_period <= 0.0) {
    ROS_WARN("[DroneRosNode] Invalid nmpc/control_period; using 0.010 s");
    config.nmpc.control_period = 0.01;
  }
  if (!std::isfinite(config.nmpc.prediction_horizon) ||
      config.nmpc.prediction_horizon <= 0.0) {
  ...
  config.nmpc.hover_thrust_ratio = xgc2_geometry::math_helpers::clamp(
      config.nmpc.hover_thrust_ratio, 0.05, 0.95);
  config.nmpc.min_hover_thrust = xgc2_geometry::math_helpers::clamp(
      config.nmpc.min_hover_thrust, 0.0, 1.0);
  config.nmpc.max_hover_thrust = xgc2_geometry::math_helpers::clamp(
  ...
  if (!std::isfinite(config.nmpc.solve_timeout) ||
      config.nmpc.solve_timeout <= 0.0) {
    ROS_WARN("[DroneRosNode] Invalid nmpc/solve_timeout; using 0.030 s");
    config.nmpc.solve_timeout = 0.03;
  }
  if (!std::isfinite(config.nmpc.result_timeout) ||
      config.nmpc.result_timeout <= 0.0) {
    ROS_WARN("[DroneRosNode] Invalid nmpc/result_timeout; using 0.100 s");
    config.nmpc.result_timeout = 0.1;
  }
  if (!std::isfinite(config.nmpc.reference_timeout) ||
      config.nmpc.reference_timeout <= 0.0) {
    ROS_WARN("[DroneRosNode] Invalid nmpc/reference_timeout; using 0.500 s");
    config.nmpc.reference_timeout = 0.5;
  }
  if (!std::isfinite(config.nmpc.reference_sample_dt) ||
      config.nmpc.reference_sample_dt <= 1e-4) {
    ROS_WARN("[DroneRosNode] Invalid nmpc/reference_sample_dt; using 0.010 s");
    config.nmpc.reference_sample_dt = 0.01;
  }
  if (config.tracking_backend == TrackingBackend::NMPC_ATTITUDE_RATE) {
    if (!config.nmpc.hover_thrust_enabled) {
      ROS_WARN("[DroneRosNode] UAV NMPC requires hover thrust estimate; "
               "forcing hover_thrust/enabled=true");
      config.nmpc.hover_thrust_enabled = true;
    }
    ROS_INFO("[DroneRosNode] UAV NMPC: dt=%.3f horizon=%.3f gravity=%.4f "
             "hover=%.3f estimator=required hover_timeout=%.3f "
             "solve_timeout=%.3f reference_timeout=%.3f "
             "reference=external_topic",
             config.nmpc.control_period, config.nmpc.prediction_horizon,
             config.nmpc.gravity, config.nmpc.hover_thrust_ratio,
             config.nmpc.hover_thrust_timeout, config.nmpc.solve_timeout,
             config.nmpc.reference_timeout);
  }
  ...
  if (config.control_mode != ControlMode::PX4_CASCADE_PID) {
    ros1_utils::getParamWithLog(nh_private_, "sliding_k1",
                                config.sliding_mode.k1, "Sliding k1");
    ros1_utils::getParamWithLog(nh_private_, "sliding_k2",
                                config.sliding_mode.k2, "Sliding k2");
    ros1_utils::getParamWithLog(nh_private_, "sliding_epsilon",
                                config.sliding_mode.epsilon, "Sliding epsilon");

    // 参数验证
    if (config.sliding_mode.k1 <= 0.0 || config.sliding_mode.k2 <= 0.0 ||
        config.sliding_mode.epsilon <= 0.0) {
      ROS_ERROR("[DroneRosNode] Invalid sliding mode parameters (must be > 0), "
                "using defaults");
  ...
  // ========== 安全限制参数 ==========
  // 位置围栏
  ros1_utils::getParamWithLog(nh_private_, "fence_x_min",
                              config.safety.fence_x_min, "Fence X min (m)");
  ros1_utils::getParamWithLog(nh_private_, "fence_x_max",
                              config.safety.fence_x_max, "Fence X max (m)");
  ros1_utils::getParamWithLog(nh_private_, "fence_y_min",
                              config.safety.fence_y_min, "Fence Y min (m)");
  ros1_utils::getParamWithLog(nh_private_, "fence_y_max",
                              config.safety.fence_y_max, "Fence Y max (m)");
  ros1_utils::getParamWithLog(nh_private_, "fence_z_min",
                              config.safety.fence_z_min, "Fence Z min (m)");
  ros1_utils::getParamWithLog(nh_private_, "fence_z_max",
                              config.safety.fence_z_max, "Fence Z max (m)");
  ...
  // 速度限制
  ros1_utils::getParamWithLog(nh_private_, "max_velocity_xy",
                              config.safety.max_velocity_xy,
                              "Max velocity XY (m/s)");
  ros1_utils::getParamWithLog(nh_private_, "max_velocity_z",
                              config.safety.max_velocity_z,
                              "Max velocity Z (m/s)");

  // 加速度饱和检测
  ros1_utils::getParamWithLog(nh_private_, "acc_saturation_xy",
                              config.safety.acc_saturation_xy,
                              "Acc saturation XY (m/s²)");
  ros1_utils::getParamWithLog(nh_private_, "acc_saturation_z",
                              config.safety.acc_saturation_z,
...
void DroneRosNode::loadVrpnQualityConfig() {
  ...
  ros1_utils::getParamWithLog(nh_private_, "vrpn_quality_window_size",
                              config.window_size, "VRPN window size");
  ros1_utils::getParamWithLog(nh_private_, "vrpn_duplicate_threshold",
                              config.duplicate_threshold, "VRPN dup threshold");
  ros1_utils::getParamWithLog(nh_private_, "vrpn_jump_threshold",
                              config.jump_threshold, "VRPN jump threshold");
