...
const std::unordered_map<std::string, ::state_machine::EventId>
    kCommandEventMap = {
        {"takeoff", event_type::CMD_TAKEOFF},
        {"Takeoff", event_type::CMD_TAKEOFF},
        {"TAKEOFF", event_type::CMD_TAKEOFF},
        {"land", event_type::CMD_LAND},
        {"Land", event_type::CMD_LAND},
        {"LAND", event_type::CMD_LAND},
        {"hover", event_type::CMD_HOVER},
        {"Hover", event_type::CMD_HOVER},
        {"HOVER", event_type::CMD_HOVER},
        {"custom1", event_type::CMD_CUSTOM1},
        {"Custom1", event_type::CMD_CUSTOM1},
        {"CUSTOM1", event_type::CMD_CUSTOM1},
};
...
CommandInputProducer::CommandInputProducer(ros::NodeHandle &nh,
                                           EventSink event_sink,
                                           uint32_t queue_size)
    : event_sink_(std::move(event_sink)) {
  command_sub_ = nh.subscribe("/command", queue_size,
                              &CommandInputProducer::commandCallback, this);
}

void CommandInputProducer::commandCallback(
  ...
  ::state_machine::Event event(
      it->second, ::state_machine::EventTimestamp{ros::Time::now().toSec()});
  ...
  if (!status.ok()) {
    ROS_ERROR("[CommandInputProducer] Failed to post command event %s: %s",
              msg->data.c_str(), status.message.c_str());
    return;
  }
