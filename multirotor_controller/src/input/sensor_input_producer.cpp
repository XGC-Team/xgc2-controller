...
SensorInputProducer::SensorInputProducer(
    ros::NodeHandle &nh, SensorData &sensor_data,
    ...
    : nh_(nh), sensor_data_(sensor_data),
      vrpn_quality_stats_(vrpn_quality_stats), queue_size_(queue_size),
      event_sink_(std::move(event_sink)),
      on_state_message_(std::move(on_state_message)), stats_manager_(nh) {}

void SensorInputProducer::setVrpnQualityConfig(
    const ros1_utils::PositionQualityConfig &config) {
...
void SensorInputProducer::start() {
  ...
  stats_manager_.register_topic<geometry_msgs::TwistStamped>(
      ...
      &SensorInputProducer::velocityCallback, this,
      &sensor_data_.local_velocity_stats);
  stats_manager_.register_topic<sensor_msgs::Imu>(
      nh_, "mavros/imu/data", queue_size_, &SensorInputProducer::imuCallback,
      this, &sensor_data_.imu_stats);
  stats_manager_.register_topic<mavros_msgs::State>(
      nh_, "mavros/state", queue_size_, &SensorInputProducer::stateCallback,
      this, &sensor_data_.state_stats);
  stats_manager_.register_topic<sensor_msgs::BatteryState>(
      nh_, "mavros/battery", queue_size_, &SensorInputProducer::batteryCallback,
      this, &sensor_data_.battery_stats);
  stats_manager_.register_topic<geometry_msgs::PoseStamped>(
      nh_, vrpn_pose_topic_, queue_size_, &SensorInputProducer::vrpnPoseCallback,
      this, &sensor_data_.vrpn_pose_stats, &vrpn_quality_stats_);
  stats_manager_.register_topic<geometry_msgs::TwistStamped>(
      nh_, vrpn_twist_topic_, queue_size_,
      &SensorInputProducer::vrpnTwistCallback, this,
...
void SensorInputProducer::resetNewFlags() { stats_manager_.resetNewFlags(); }
...
void SensorInputProducer::batteryCallback(
...
void SensorInputProducer::vrpnPoseCallback(
    const geometry_msgs::PoseStamped::ConstPtr &msg) {
  ...
  vrpn_quality_stats_ = vrpn_quality_detector_.process(
      sensor_data_.vrpn_x, sensor_data_.vrpn_y, sensor_data_.vrpn_z);
  postInputEvent(event_type::INPUT_VRPN_POSE_UPDATED, "pose");
}

void SensorInputProducer::vrpnTwistCallback(
    const geometry_msgs::TwistStamped::ConstPtr &msg) {
...
void SensorInputProducer::postInputEvent(::state_machine::EventId event_id,
                                         const char *source) {
  if (!event_sink_) {
  ...
  ::state_machine::Event event(
      event_id, ::state_machine::EventTimestamp{ros::Time::now().toSec()});
  event.source = source;
