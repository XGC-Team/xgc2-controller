...
bool UavNmpcSolver::initialize() {
    ...
    if (UAV_NMPC_NX != 13 || UAV_NMPC_NU != 4 || UAV_NMPC_NP != 17 ||
        UAV_NMPC_N != 10 || UAV_NMPC_NH != 1 || UAV_NMPC_NHN != 1 ||
        UAV_NMPC_NSBU != 4 || UAV_NMPC_NSBX != 9 ||
        UAV_NMPC_NSH != 1 || UAV_NMPC_NS != 14 ||
        UAV_NMPC_NS0 != 4 || UAV_NMPC_NSBXN != 9 ||
    ...
    initialized_ = true;
    resetWarmStart();
    ROS_INFO("[UavNmpcSolver] Initialized generated acados solver (N=%d, nx=%d, nu=%d)",
             UAV_NMPC_N, UAV_NMPC_NX, UAV_NMPC_NU);
    return true;
}
...
bool UavNmpcSolver::solve(const Vector13d& x0,
                          const std::vector<UavReferenceSample>& references) {
    if (!initialized_ && !initialize()) {
        return false;
    }
    if (references.size() != static_cast<size_t>(UAV_NMPC_N + 2)) {
        ROS_ERROR("[UavNmpcSolver] Expected %d references, got %zu",
                  UAV_NMPC_N + 2, references.size());
        return false;
    }
    if (!isFinite(x0)) {
