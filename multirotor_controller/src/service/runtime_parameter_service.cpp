...
const std::unordered_map<std::string, DoubleSetter> &doubleSetters() {
  static const std::unordered_map<std::string, DoubleSetter> setters = {
      ...
      {"nmpc/gravity",
       [](ControllerConfig &cfg, double value) { cfg.nmpc.gravity = value; }},
      ...
      {"nmpc/reference_sample_dt",
       [](ControllerConfig &cfg, double value) { cfg.nmpc.reference_sample_dt = value; }},
      {"nmpc/log_period",
      ...
      {"safety/position_jump_threshold",
       [](ControllerConfig &cfg, double value) {
         cfg.safety.position_jump_threshold = value;
       }},
  };
  return setters;
}
...
const std::unordered_map<std::string, BoolSetter> &boolSetters() {
  static const std::unordered_map<std::string, BoolSetter> setters = {
      {"skip_takeoff_init_disarm",
       [](ControllerConfig &cfg, bool value) {
         cfg.skip_takeoff_init_disarm = value;
       }},
      {"enable_yaw_control",
       [](ControllerConfig &cfg, bool value) { cfg.enable_yaw_control = value; }},
      {"nmpc/hover_thrust_enabled",
       [](ControllerConfig &cfg, bool value) {
         cfg.nmpc.hover_thrust_enabled = value;
       }},
      {"nmpc/enable_timing_log",
       [](ControllerConfig &cfg, bool value) {
         cfg.nmpc.enable_timing_log = value;
       }},
  };
  return setters;
}
...
RuntimeParameterService::RuntimeParameterService(ros::NodeHandle &nh,
                                                 DroneController &controller)
    : controller_(controller) {
  service_ = nh.advertiseService("set_runtime_parameters",
                                 &RuntimeParameterService::handle, this);
...
bool RuntimeParameterService::handle(SetRuntimeParameters::Request &request,
  ...
  ROS_INFO("[RuntimeParameterService] Updated %zu runtime parameter(s)",
           request.names.size());
  return true;
}

bool RuntimeParameterService::applyParameter(ControllerConfig &config,
                                             const std::string &name,
                                             const std::string &value,
                                             std::string &error) const {
  if (name == "nmpc/prediction_horizon") {
    error = "nmpc/prediction_horizon is generated into the acados solver and "
...
bool RuntimeParameterService::validate(const ControllerConfig &config,
                                       std::string &error) const {
  ...
  if (!positive(config.takeoff_altitude, "takeoff_altitude") ||
      ...
      !positive(config.nmpc.hover_thrust_timeout,
                "nmpc/hover_thrust_timeout") ||
  ...
  if (!std::isfinite(config.nmpc.reference_sample_dt) ||
    ...
    error = "nmpc/reference_sample_dt must be finite and > 1e-4";
    return false;
  }
  if (config.nmpc.hover_thrust_ratio < 0.0 ||
      config.nmpc.hover_thrust_ratio > 1.0 ||
      config.nmpc.min_hover_thrust < 0.0 ||
      config.nmpc.max_hover_thrust > 1.0 ||
  ...
  if (!positive(config.safety.max_velocity_xy, "safety/max_velocity_xy") ||
      ...
      !positive(config.safety.position_jump_threshold,
                "safety/position_jump_threshold")) {
    return false;
  }
  return true;
...
RuntimeParameterService::currentValues(const ControllerConfig &config) const {
  return {
      ...
      "nmpc/hover_thrust_ratio=" +
          formatDouble(config.nmpc.hover_thrust_ratio),
      ...
      "nmpc/specific_thrust_min=" +
          formatDouble(config.nmpc.specific_thrust_min),
      "nmpc/specific_thrust_max=" +
          formatDouble(config.nmpc.specific_thrust_max),
      "nmpc/hover_thrust_enabled=" +
          formatBool(config.nmpc.hover_thrust_enabled),
      "nmpc/hover_thrust_timeout=" +
          formatDouble(config.nmpc.hover_thrust_timeout),
      ...
      "nmpc/reference_sample_dt=" +
          formatDouble(config.nmpc.reference_sample_dt),
      ...
      "safety/acc_saturation_xy=" +
          formatDouble(config.safety.acc_saturation_xy),
      "safety/acc_saturation_z=" +
          formatDouble(config.safety.acc_saturation_z),
      "safety/position_jump_threshold=" +
          formatDouble(config.safety.position_jump_threshold),
  };
}
