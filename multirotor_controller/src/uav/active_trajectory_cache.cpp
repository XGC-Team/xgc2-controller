#include "multirotor_controller/uav/active_trajectory_cache.h"
...
reference_trajectory::core::AnalyticType analyticType(uint16_t value) {
    switch (value) {
        case reference_trajectory::AnalyticReference::ANALYTIC_HOLD:
            return reference_trajectory::core::AnalyticType::kHold;
        case reference_trajectory::AnalyticReference::ANALYTIC_CIRCLE:
            return reference_trajectory::core::AnalyticType::kCircle;
        case reference_trajectory::AnalyticReference::ANALYTIC_HEIGHT_CIRCLE:
            return reference_trajectory::core::AnalyticType::kHeightCircle;
        case reference_trajectory::AnalyticReference::ANALYTIC_CIRCLE_ENTRY:
            return reference_trajectory::core::AnalyticType::kCircleEntry;
        case reference_trajectory::AnalyticReference::ANALYTIC_FIGURE_EIGHT:
            return reference_trajectory::core::AnalyticType::kFigureEight;
        default:
            return reference_trajectory::core::AnalyticType::kCircleEntry;
    }
}
...
bool fatalReferenceFlags(uint32_t flags) {
    constexpr uint32_t kFatal = reference_trajectory::core::kFlagInvalidInput |
                                reference_trajectory::core::kFlagNonFinite |
                                reference_trajectory::core::kFlagLowThrust |
                                reference_trajectory::core::kFlagYawSingularity;
...
bool ActiveTrajectoryCache::updateAnalytic(const reference_trajectory::AnalyticReference& msg,
                                           const ros::Time& received_time) {
    auto evaluator = std::make_unique<reference_trajectory::core::AnalyticEvaluator>();
    uint32_t flags = 0U;
    ...
    type_ = reference_trajectory::core::TrajectoryModelType::kAnalytic;
...
bool ActiveTrajectoryCache::updatePolynomial(
    const reference_trajectory::ActivePolynomialReference& msg, const ros::Time& received_time) {
    auto evaluator = std::make_unique<reference_trajectory::core::PiecewisePolynomialEvaluator>();
    uint32_t flags = 0U;
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    evaluator_ = std::move(evaluator);
    type_ = reference_trajectory::core::TrajectoryModelType::kPolynomial;
    trajectory_id_ = msg.trajectory_id;
    revision_ = msg.revision;
    ++sequence_;
    start_time_ = msg.start_time.isZero() ? msg.header.stamp : msg.start_time;
...
bool ActiveTrajectoryCache::updateSampled(const reference_trajectory::SampledReference& msg,
                                          const ros::Time& received_time) {
    auto evaluator = std::make_unique<reference_trajectory::core::SampledEvaluator>();
    uint32_t flags = 0U;
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    evaluator_ = std::move(evaluator);
    type_ = reference_trajectory::core::TrajectoryModelType::kSampled;
    trajectory_id_ = msg.trajectory_id;
    revision_ = msg.revision;
    ++sequence_;
    start_time_ = msg.start_time.isZero() ? msg.header.stamp : msg.start_time;
...
void ActiveTrajectoryCache::clear() {
    std::lock_guard<std::mutex> lock(mutex_);
    evaluator_.reset();
    type_ = reference_trajectory::core::TrajectoryModelType::kNone;
    trajectory_id_ = 0U;
    revision_ = 0U;
    sequence_ = 0U;
...
bool ActiveTrajectoryCache::sample(const ros::Time& now, double timeout,
    ...
    std::unique_ptr<reference_trajectory::core::TrajectoryEvaluator> evaluator;
    ...
    ros::Time local_received;
    {
        ...
        evaluator = reference_trajectory::core::cloneEvaluator(*evaluator_);
...
bool ActiveTrajectoryCache::sampleHorizon(
    ...
    reference_trajectory::core::FlatnessMapper mapper(gravity, 0.1);
    references.clear();
...
bool ActiveTrajectoryCache::buildAnalyticEvaluator(
    const reference_trajectory::AnalyticReference& msg,
    reference_trajectory::core::AnalyticEvaluator& evaluator, uint32_t& flags) {
    flags = msg.flags;
    ...
    evaluator = reference_trajectory::core::AnalyticEvaluator(params);
    flags |= reference_trajectory::core::TrajectoryValidator::validate(
        evaluator, reference_trajectory::core::TrajectoryLimits{}, 0.02);
...
bool ActiveTrajectoryCache::buildPolynomialEvaluator(
    ...
    if (msg.order < 1U || segment_count == 0U ||
        ...
        flags |= reference_trajectory::core::kFlagInvalidInput;
        return false;
    }
    const bool has_yaw = msg.coeff_yaw.size() == segment_count * coeffs_per_segment;
    std::vector<reference_trajectory::core::PolynomialSegment> segments;
    segments.reserve(segment_count);
    for (size_t i = 0; i < segment_count; ++i) {
        reference_trajectory::core::PolynomialSegment segment;
...
bool ActiveTrajectoryCache::buildSampledEvaluator(
    const reference_trajectory::SampledReference& msg,
    reference_trajectory::core::SampledEvaluator& evaluator, uint32_t& flags) {
    flags = msg.flags;
    std::vector<reference_trajectory::core::SampledPoint> samples;
    samples.reserve(msg.points.size());
    for (const auto& point : msg.points) {
        reference_trajectory::core::SampledPoint sample;
...
UavReferencePoint ActiveTrajectoryCache::toPoint(const reference_trajectory::core::FlatOutput& flat,
                                                 double t) {
...
reference_trajectory::UavReferenceSample ActiveTrajectoryCache::toNmpcSample(
    ...
    sample.x.segment<4>(6) = reference_trajectory::quatToVecWxyz(full.attitude);
