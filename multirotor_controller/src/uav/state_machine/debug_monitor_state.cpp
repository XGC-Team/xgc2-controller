...
DebugMonitorState::DebugMonitorState(DroneController &controller)
    : controller_(controller) {}

::state_machine::ActionResult
DebugMonitorState::onTick(::state_machine::StateContext &ctx) {
  ...
  if (!debug_print_initialized_ ||
      now - last_debug_print_time_ >= DEBUG_PRINT_INTERVAL) {
...
void DebugMonitorState::emit(::state_machine::StateContext &ctx,
                             ::state_machine::EventId event_id,
                             double timestamp) const {
  ::state_machine::Event event(event_id,
                               ::state_machine::EventTimestamp{timestamp});
  event.source = "debug";
