#include "multirotor_controller/uav/state_machine/takeoff_offboard_request_state.h"
...
TakeoffOffboardRequestState::TakeoffOffboardRequestState(
    DroneController &controller)
    : controller_(controller) {}

::state_machine::ActionResult
TakeoffOffboardRequestState::onEnter(::state_machine::StateContext &ctx) {
  controller_.logInfo(
      "[TakeoffOffboardRequestState] Entering OFFBOARD Request State");
  ...
  controller_.logInfo(
      "[TakeoffOffboardRequestState] Ready to request OFFBOARD mode");
  return {};
}

::state_machine::ActionResult
TakeoffOffboardRequestState::onTick(::state_machine::StateContext &ctx) {
...
void TakeoffOffboardRequestState::publishSetpointIfDue(
...
void TakeoffOffboardRequestState::requestOffboardModeIfDue(
    ::state_machine::StateContext &ctx) {
  ...
  controller_.logInfo(
      "[TakeoffOffboardRequestState] Requesting OFFBOARD mode (periodic)");
  ::state_machine::Event event(
      output_event_type::REQUEST_MODE,
      ::state_machine::EventTimestamp{controller_.getCurrentTime()});
...
void TakeoffOffboardRequestState::updateOffboardConfirmation() {
  ...
  if (sensor_data.fcu_mode == "OFFBOARD") {
    ...
    controller_.logInfo(
        "[TakeoffOffboardRequestState] Received OFFBOARD frame %d/%d",
...
void TakeoffOffboardRequestState::postOffboardReadyOnce(
    ::state_machine::StateContext &ctx) {
  if (offboard_ready_event_posted_ ||
      confirmed_offboard_frames_ < REQUIRED_OFFBOARD_FRAMES) {
    return;
  }

  controller_.logInfo(
      "[TakeoffOffboardRequestState] OFFBOARD mode ready (%d frames received)",
      confirmed_offboard_frames_);
  offboard_ready_event_posted_ = true;
  ctx.postInternalEvent(::state_machine::Event(
      event_type::OFFBOARD_READY,
      ::state_machine::EventTimestamp{controller_.getCurrentTime()}));
}

::state_machine::ActionResult
TakeoffOffboardRequestState::onExit(::state_machine::StateContext &ctx) {
  const auto &sensor_data = controller_.getSensorData();
  const double elapsed_time =
      std::chrono::duration<double>(
          ctx.elapsed(state_type::TakeoffOffboardRequest))
          .count();
  controller_.logInfo(
