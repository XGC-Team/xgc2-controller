#include "multirotor_controller/uav/state_machine/custom1_state.h"
...
Custom1State::Custom1State(DroneController &controller)
    : controller_(controller) {}

::state_machine::ActionResult
Custom1State::onEnter(::state_machine::StateContext &ctx) {
  // 获取配置
  const auto &config = controller_.getConfig();
  trajectory_lifter_ =
      TrajectoryLifter(TrajectoryLifterConfig{config.planning_period});

  if (config.tracking_backend == TrackingBackend::NMPC_ATTITUDE_RATE) {
    controller_.logInfo(
        "[Custom1State] Entering UAV NMPC Attitude-Rate Tracking Mode");
    controller_.referenceTrajectoryBuffer().clear();
    request_sequence_ = 0;
    ...
    if (!status.ok()) {
      controller_.logWarn(
          "[Custom1State] Failed to emit reference activation event: %s",
  ...
  if (config.control_mode != ControlMode::PX4_CASCADE_PID) {
    sliding_controller_.setParams(config.sliding_mode.k1,
                                  config.sliding_mode.k2,
                                  config.sliding_mode.epsilon);
    controller_.logInfo(
        "[Custom1State] Sliding mode params: k1=%.2f, k2=%.2f, eps=%.3f",
...
::state_machine::ActionResult
Custom1State::onTick(::state_machine::StateContext &ctx) {
  const double current_time = controller_.getCurrentTime();
  if (controller_.getConfig().tracking_backend ==
  ...
  reference_trajectory_ =
      trajectory_lifter_.lift(mpc_traj, ros::Time(current_time));
  ...
  ctx.emitOutput(::state_machine::Event(
      output_event_type::PUBLISH_SETPOINT,
      ::state_machine::EventTimestamp{controller_.getCurrentTime()}));
...
void Custom1State::handleNmpcEventMode(::state_machine::StateContext &ctx,
                                       double current_time) {
  ...
  if (request_in_flight_ && current_time > request_deadline_) {
    ...
    request_in_flight_ = false;
    ::state_machine::Event timeout_event(
        event_type::INPUT_NMPC_SOLVE_TIMED_OUT,
  ...
  if (!controller_.referenceTrajectoryBuffer().valid(
          ros::Time(current_time),
          controller_.getConfig().nmpc.reference_timeout)) {
...
void Custom1State::consumeNmpcResult(::state_machine::StateContext &ctx,
                                     double current_time) {
  NmpcSolveResult result;
  if (!controller_.nmpcResultBuffer().consumeNewerThan(consumed_result_sequence_,
                                                       result)) {
    return;
  }
  ...
  if (!result.success) {
    ...
    if (last_success_time_ > 0.0 &&
        current_time - last_success_time_ <=
            controller_.getConfig().nmpc.result_timeout) {
      ctx.emitOutput(::state_machine::Event(
          output_event_type::PUBLISH_ATTITUDE_RATE_TARGET,
          ::state_machine::EventTimestamp{current_time}));
      return;
    }
    publishBackupSetpoint(ctx, current_time);
...
void Custom1State::dispatchNmpcRequest(::state_machine::StateContext &ctx,
                                       double current_time) {
  ++request_sequence_;
  ...
  ::state_machine::Event event(
      output_event_type::REQUEST_NMPC_SOLVE,
      ::state_machine::EventTimestamp{current_time});
...
void Custom1State::publishBackupSetpoint(::state_machine::StateContext &ctx,
                                         double current_time) {
  ...
  if (controller_.referenceTrajectoryBuffer().sample(
    ...
    backup.ay = 0.0;
    backup.az = 0.0;
    backup.type_mask = 0b110111000000;
  } else {
    const auto &sensor = controller_.getSensorData();
    backup.x = sensor.x;
    backup.y = sensor.y;
    backup.z = sensor.z;
    backup.type_mask = 0b111111111000;
  }
...
bool Custom1State::updateActiveMpcFrame(double current_time) {
  ...
  if (!active_traj.is_valid) {
    ...
    const ros::Time activation_time = pending_traj.planning_time.isZero()
                                          ? current_ros_time
  ...
  const double active_age =
      (current_ros_time - active_traj.planning_time).toSec();
...
void Custom1State::computeTrackingErrors(const SensorData &sensor_data,
                                         Eigen::Vector3d &e_p,
...
void Custom1State::computeControlOutput() {
  ...
  case ControlMode::PX4_CASCADE_PID: {
    ...
    control_output_ = reference_trajectory_;
    break;
  }
  ...
  case ControlMode::HYBRID_CONTROL: {
    ...
    // 提取MPC前馈加速度 u_h
    Eigen::Vector3d u_h(reference_trajectory_.ax, reference_trajectory_.ay,
                        reference_trajectory_.az);
...
::state_machine::ActionResult
Custom1State::onExit(::state_machine::StateContext &) {
  ...
  if (controller_.getConfig().tracking_backend ==
      TrackingBackend::NMPC_ATTITUDE_RATE) {
    controller_.logInfo(
        "[Custom1State] Exiting UAV NMPC Attitude-Rate Tracking Mode");
    request_in_flight_ = false;
    return {};
  }
