...
::state_machine::ActionResult Custom1State::onTick(::state_machine::StateContext& ctx) {
    ...
    // 1. 生成参考轨迹（MPC插值结果，只读）
    reference_trajectory_ = trajectory_lifter_.lift(mpc_traj, ros::Time(current_time));
...
void Custom1State::computeTrackingErrors(const SensorData& sensor_data, Eigen::Vector3d& e_p,
    ...
    e_p << sensor_data.x - reference_trajectory_.x, sensor_data.y - reference_trajectory_.y,
        sensor_data.z - reference_trajectory_.z;

    e_v << sensor_data.vx - reference_trajectory_.vx, sensor_data.vy - reference_trajectory_.vy,
