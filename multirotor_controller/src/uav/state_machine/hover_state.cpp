...
HoverState::HoverState(DroneController &controller)
    : controller_(controller) {}

::state_machine::ActionResult
HoverState::onEnter(::state_machine::StateContext &ctx) {
...
void HoverState::configureHoverSetpoint() {
  ...
  controller_.logInfo("[HoverState] Holding position (%.2f, %.2f, %.2f)",
                      setpoint.x, setpoint.y, setpoint.z);
}

::state_machine::ActionResult
HoverState::onTick(::state_machine::StateContext &ctx) {
...
::state_machine::ActionResult
HoverState::onExit(::state_machine::StateContext &) {
