...
TakeoffArmRequestState::onEnter(::state_machine::StateContext &ctx) {
...
::state_machine::ActionResult
TakeoffArmRequestState::onTick(::state_machine::StateContext &ctx) {
...
void TakeoffArmRequestState::publishSetpointIfDue(
...
void TakeoffArmRequestState::requestArmIfDue(
    ::state_machine::StateContext &ctx) {
  ...
  controller_.logInfo(
      "[TakeoffArmRequestState] Requesting ARM (periodic, armed: %s)",
...
void TakeoffArmRequestState::updateArmConfirmation() {
  ...
  if (confirmed_arm_frames_ > 0) {
    controller_.logWarn(
        "[TakeoffArmRequestState] FCU disarmed, resetting ARM counter (was %d)",
        confirmed_arm_frames_);
    confirmed_arm_frames_ = 0;
  }
}

void TakeoffArmRequestState::postArmReadyOnce(
    ::state_machine::StateContext &ctx) {
  ...
  controller_.logInfo(
      "[TakeoffArmRequestState] ARM ready (%d frames received)",
      confirmed_arm_frames_);
  arm_ready_event_posted_ = true;
  ctx.postInternalEvent(::state_machine::Event(
      event_type::ARM_READY,
      ::state_machine::EventTimestamp{controller_.getCurrentTime()}));
}

::state_machine::ActionResult
TakeoffArmRequestState::onExit(::state_machine::StateContext &ctx) {
  const auto &sensor_data = controller_.getSensorData();
  const double elapsed_time =
      std::chrono::duration<double>(
          ctx.elapsed(state_type::TakeoffArmRequest))
          .count();
  controller_.logInfo(
      "[TakeoffArmRequestState] Exiting (armed: %s, ARM frames: %d, "
      "elapsed: %.2f s)",
      sensor_data.fcu_armed ? "true" : "false", confirmed_arm_frames_,
      elapsed_time);
