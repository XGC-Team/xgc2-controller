...
LandingState::LandingState(DroneController &controller)
    : controller_(controller) {}

::state_machine::ActionResult
LandingState::onEnter(::state_machine::StateContext &ctx) {
  ...
  const double slowest_descent_rate =
      std::max(std::min(std::abs(LANDING_VZ_HIGH_ALTITUDE),
                        std::abs(LANDING_VZ_LOW_ALTITUDE)),
               1e-3);
  ...
  controller_.logInfo(
      "[LandingState] Initial altitude: %.2f m, max duration: %.1f s",
...
::state_machine::ActionResult
LandingState::onTick(::state_machine::StateContext &ctx) {
...
void LandingState::postTouchdownOnce(::state_machine::StateContext &ctx) {
  if (touchdown_event_posted_ ||
      confirmed_landed_frames_ < CONSECUTIVE_SETTLED_FRAMES) {
    return;
  }
...
::state_machine::ActionResult
LandingState::onExit(::state_machine::StateContext &ctx) {
