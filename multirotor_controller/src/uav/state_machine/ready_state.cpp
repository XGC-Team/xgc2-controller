...
ReadyState::ReadyState(DroneController &controller)
    : controller_(controller) {}

::state_machine::ActionResult
ReadyState::onEnter(::state_machine::StateContext &ctx) {
  ...
  if (sensor_checks::isAirborne(sensor_data)) {
    controller_.logWarn(
        "[ReadyState] Airborne flag set, skipping disarm request");
  } else {
...
::state_machine::ActionResult
ReadyState::onTick(::state_machine::StateContext &) {
  // 就绪状态无需周期性控制输出，等待外部命令或 guard 转移。
  return {};
}

::state_machine::ActionResult
ReadyState::onExit(::state_machine::StateContext &) {
