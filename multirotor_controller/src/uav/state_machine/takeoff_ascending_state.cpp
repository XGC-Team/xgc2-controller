...
TakeoffAscendingState::onEnter(::state_machine::StateContext &ctx) {
...
::state_machine::ActionResult
TakeoffAscendingState::onTick(::state_machine::StateContext &ctx) {
...
void TakeoffAscendingState::publishSetpointIfDue(
...
bool TakeoffAscendingState::postTimeoutIfNeeded(
    ::state_machine::StateContext &ctx) {
  ...
  controller_.logWarn(
      "[TakeoffAscendingState] Ascending timeout after %.1f seconds",
      elapsed_time);
  timeout_event_posted_ = true;
  ctx.postInternalEvent(::state_machine::Event(
      event_type::TAKEOFF_TIMEOUT,
      ::state_machine::EventTimestamp{controller_.getCurrentTime()}));
  return true;
}
...
void TakeoffAscendingState::postAltitudeReachedOnce(
    ::state_machine::StateContext &ctx) {
  if (altitude_reached_event_posted_ ||
      confirmed_settled_frames_ < CONSECUTIVE_SETTLED_FRAMES) {
    return;
  }
  ...
  ctx.postInternalEvent(::state_machine::Event(
      event_type::ALTITUDE_REACHED,
      ::state_machine::EventTimestamp{controller_.getCurrentTime()}));
}

::state_machine::ActionResult
TakeoffAscendingState::onExit(::state_machine::StateContext &ctx) {
  const auto &sensor_data = controller_.getSensorData();
  const double elapsed_time =
      std::chrono::duration<double>(ctx.elapsed(state_type::TakeoffAscending))
          .count();
