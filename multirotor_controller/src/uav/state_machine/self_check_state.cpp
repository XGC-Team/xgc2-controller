...
SelfCheckState::SelfCheckState(DroneController &controller)
    : controller_(controller) {}

::state_machine::ActionResult
SelfCheckState::onEnter(::state_machine::StateContext &) {
...
::state_machine::ActionResult
SelfCheckState::onTick(::state_machine::StateContext &) {
  // 每2秒打印一次传感器状态
  if (status_log_timer_.elapsed().count() >= LOG_INTERVAL) {
    ...
    controller_.logInfo(
        ...
        sensor_data.battery_stats.is_active ? "OK" : "X",
        vrpn_pose_active ? "OK" : "X", vrpn_twist_active ? "OK" : "X",
        pose_consistent ? "OK" : "X", vrpn_local_diff,
...
::state_machine::ActionResult
SelfCheckState::onExit(::state_machine::StateContext &) {
