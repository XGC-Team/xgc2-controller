#include "multirotor_controller/uav/state_machine/takeoff_init_state.h"
...
TakeoffInitState::TakeoffInitState(DroneController &controller)
    : controller_(controller) {}

::state_machine::ActionResult
TakeoffInitState::onEnter(::state_machine::StateContext &ctx) {
...
void TakeoffInitState::configureTakeoffSetpoint() {
  ...
  controller_.logInfo(
      "[TakeoffInitState] Initial altitude: %.2f m, Target: %.2f m",
      initial_altitude_, target_altitude_);
}

void TakeoffInitState::requestDisarmIfRequired(
...
::state_machine::ActionResult
TakeoffInitState::onTick(::state_machine::StateContext &ctx) {
...
void TakeoffInitState::requestAltctlModeIfDue(
    ::state_machine::StateContext &ctx) {
...
void TakeoffInitState::updateAltctlConfirmation() {
  ...
  if (sensor_data.fcu_mode == "ALTCTL") {
    altctl_frame_count_ =
        std::min(altctl_frame_count_ + 1, REQUIRED_ALTCTL_FRAMES);
    controller_.logInfo(
        "[TakeoffInitState] Received ALTCTL frame %d/%d",
        altctl_frame_count_, REQUIRED_ALTCTL_FRAMES);
    return;
  }
...
void TakeoffInitState::postAltctlReadyOnce(
    ::state_machine::StateContext &ctx) {
  if (altctl_ready_event_posted_ ||
      altctl_frame_count_ < REQUIRED_ALTCTL_FRAMES) {
    return;
  }
  ...
  if (skip_altctl_gate_) {
    controller_.logInfo(
        "[TakeoffInitState] ALTCTL mode request/check skipped by parameter");
  } else {
    controller_.logInfo(
        "[TakeoffInitState] ALTCTL mode ready (%d frames received)",
        altctl_frame_count_);
  }

  altctl_ready_event_posted_ = true;
  ctx.postInternalEvent(::state_machine::Event(
      event_type::ALTCTL_READY,
      ::state_machine::EventTimestamp{controller_.getCurrentTime()}));
}

::state_machine::ActionResult
TakeoffInitState::onExit(::state_machine::StateContext &ctx) {
  const double elapsed_time =
      std::chrono::duration<double>(ctx.elapsed(state_type::TakeoffInit))
          .count();
  controller_.logInfo(
