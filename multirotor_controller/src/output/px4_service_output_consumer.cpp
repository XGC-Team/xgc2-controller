...
#include "multirotor_controller/common/px4_command.h"
...
void Px4ServiceOutputConsumer::initializeClientsIfNeeded() {
  ...
  arming_client_ =
      nh_.serviceClient<mavros_msgs::CommandLong>("mavros/cmd/command", true);
  set_mode_client_ =
      nh_.serviceClient<mavros_msgs::SetMode>("mavros/set_mode", true);
...
bool Px4ServiceOutputConsumer::handle(const ::state_machine::Event &event) {
  ...
  case output_event_type::REQUEST_ARMING: {
    ...
    if (it == event.payload.end() ||
      ...
      ROS_WARN("[Px4ServiceOutputConsumer] REQUEST_ARMING missing bool "
               "payload 'arm'");
      return true;
    }
    if (!clients_initialized_) {
      ROS_WARN("[Px4ServiceOutputConsumer] Arming client not initialized, "
               "dropping arm request");
      return true;
    }
    executor_.pushTask(std::make_unique<ArmOutputTask>(
        std::get<bool>(it->second), &arming_client_));
    return true;
  }
  ...
  case output_event_type::REQUEST_MODE: {
    ...
    if (it == event.payload.end() ||
      ...
      ROS_WARN("[Px4ServiceOutputConsumer] REQUEST_MODE missing string "
               "payload 'mode'");
      return true;
    }
    if (!clients_initialized_) {
      ROS_WARN("[Px4ServiceOutputConsumer] SetMode client not initialized, "
