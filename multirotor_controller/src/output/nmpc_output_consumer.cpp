...
NmpcOutputConsumer::NmpcOutputConsumer(DroneController &controller,
...
bool NmpcOutputConsumer::handle(const ::state_machine::Event &event) {
  ...
  const ros::Time now(event.timestamp > 0.0 ? event.timestamp
                                            : ros::Time::now().toSec());
  Request request;
  ...
  const double stage_dt = config.nmpc.prediction_horizon /
                          static_cast<double>(UavNmpcSolver::horizonSteps());
  if (!controller_.referenceTrajectoryBuffer().sampleHorizon(
          now, stage_dt, UavNmpcSolver::horizonSteps(),
          config.nmpc.reference_timeout, config.nmpc.gravity,
          request.references)) {
    reject(sequence, -2);
    return true;
  }
...
void NmpcOutputConsumer::workerLoop() {
  ...
  while (true) {
    ...
    if (entered) {
      result.success =
          backend_.compute(request.sensor, request.references, request.now,
                           result.target);
...
void NmpcOutputConsumer::postResultEvent(uint64_t sequence, bool success) {
  ...
  ::state_machine::Event event(
      success ? event_type::INPUT_NMPC_SOLVE_SUCCEEDED
              : event_type::INPUT_NMPC_SOLVE_FAILED,
