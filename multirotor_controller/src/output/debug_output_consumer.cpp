...
DebugOutputConsumer::DebugOutputConsumer(
    ...
    : executor_(executor), controller_(controller), sensor_data_(sensor_data),
  ...
  stats_local_vel_pub_ = nh.advertise<std_msgs::Float32MultiArray>(
      "alg/stats/local_velocity", queue_size);
  stats_imu_pub_ =
      nh.advertise<std_msgs::Float32MultiArray>("alg/stats/imu", queue_size);
  stats_state_pub_ =
      nh.advertise<std_msgs::Float32MultiArray>("alg/stats/state", queue_size);
  stats_battery_pub_ = nh.advertise<std_msgs::Float32MultiArray>(
      "alg/stats/battery", queue_size);
  stats_vrpn_pose_pub_ = nh.advertise<std_msgs::Float32MultiArray>(
      "alg/stats/vrpn_pose", queue_size);
  stats_vrpn_twist_pub_ = nh.advertise<std_msgs::Float32MultiArray>(
      "alg/stats/vrpn_twist", queue_size);
  vrpn_quality_pub_ =
      nh.advertise<std_msgs::Float32MultiArray>("alg/vrpn_quality", queue_size);
  tracking_error_pub_ = nh.advertise<std_msgs::Float32MultiArray>(
      "alg/tracking/position_error", queue_size);
}
...
bool DebugOutputConsumer::handle(const ::state_machine::Event &event) {
  ...
  case output_event_type::PUBLISH_CONTROLLER_STATUS: {
    std_msgs::String msg;
    msg.data =
        controller_.getStateMachine().currentStateName(region_type::CONTROL);
    if (msg.data.empty()) {
      msg.data = "Unknown";
    }
    executor_.pushTask(makePublishTask("PublishControllerStatus",
                                       controller_status_pub_, std::move(msg)));
    return true;
  }
  ...
  case output_event_type::PUBLISH_TRACKING_ERROR:
    executor_.pushTask(makePublishTask("PublishTrackingError",
                                       tracking_error_pub_,
...
DebugOutputConsumer::snapshotTrackingError() const {
  std_msgs::Float32MultiArray msg;
  msg.data.reserve(10);

  UavReferencePoint reference;
  const ros::Time now = ros::Time::now();
  if (!controller_.referenceTrajectoryBuffer().sample(
...
DebugOutputConsumer::snapshotStateMachineEvents() const {
...
void DebugOutputConsumer::printSensorDebug(const SensorData &sensor_data) {
  ...
  ROS_INFO("[SensorData Debug] Velocity: vx=%.3f, vy=%.3f, vz=%.3f",
           sensor_data.vx, sensor_data.vy, sensor_data.vz);
  ROS_INFO("[SensorData Debug] Angular velocity: wx=%.3f, wy=%.3f, wz=%.3f",
           sensor_data.wx, sensor_data.wy, sensor_data.wz);
  ROS_INFO("[SensorData Debug] Battery: %.1f%%",
           sensor_data.battery_percentage * 100.0);
  ROS_INFO("[SensorData Debug] LocalPos: %.1f Hz, dt_max=%.6f s, jitter=%.6f "
           "s, time_since_last=%.3f s, active=%s",
           sensor_data.local_pos_stats.frequency_hz,
           sensor_data.local_pos_stats.dt_max,
           sensor_data.local_pos_stats.jitter,
           sensor_data.local_pos_stats.time_since_last_msg,
           sensor_data.local_pos_stats.is_active ? "true" : "false");
  ROS_INFO("[SensorData Debug] LocalVel: %.1f Hz, dt_max=%.6f s, jitter=%.6f "
           "s, time_since_last=%.3f s, active=%s",
           sensor_data.local_velocity_stats.frequency_hz,
           sensor_data.local_velocity_stats.dt_max,
  ...
  ROS_INFO("[SensorData Debug] IMU: %.1f Hz, dt_max=%.6f s, jitter=%.6f s, "
           "time_since_last=%.3f s, active=%s",
           sensor_data.imu_stats.frequency_hz, sensor_data.imu_stats.dt_max,
           sensor_data.imu_stats.jitter,
           sensor_data.imu_stats.time_since_last_msg,
           sensor_data.imu_stats.is_active ? "true" : "false");
  ROS_INFO("[SensorData Debug] State: %.1f Hz, dt_max=%.6f s, jitter=%.6f s, "
           "time_since_last=%.3f s, active=%s",
           sensor_data.state_stats.frequency_hz, sensor_data.state_stats.dt_max,
           sensor_data.state_stats.jitter,
           sensor_data.state_stats.time_since_last_msg,
           sensor_data.state_stats.is_active ? "true" : "false");
  ROS_INFO("[SensorData Debug] Battery: %.1f Hz, dt_max=%.6f s, jitter=%.6f s, "
           "time_since_last=%.3f s, active=%s",
           sensor_data.battery_stats.frequency_hz,
           sensor_data.battery_stats.dt_max, sensor_data.battery_stats.jitter,
           sensor_data.battery_stats.time_since_last_msg,
           sensor_data.battery_stats.is_active ? "true" : "false");
  ROS_INFO("[SensorData Debug] FCU: connected=%s, armed=%s, guided=%s, "
           "manual_input=%s, mode=%s, status=%u",
           sensor_data.fcu_connected ? "true" : "false",
           sensor_data.fcu_armed ? "true" : "false",
           sensor_data.fcu_guided ? "true" : "false",
           sensor_data.fcu_manual_input ? "true" : "false",
           sensor_data.fcu_mode.c_str(),
           static_cast<unsigned>(sensor_data.fcu_system_status));
  ROS_INFO("[SensorData Debug] ================================");
}
