...
ReferenceActivationOutputConsumer::ReferenceActivationOutputConsumer(
    ...
    activation_pub_ = nh.advertise<reference_trajectory::AnalyticReference>(
        "alg/reference_trajectory/request/analytic", queue_size);
}
...
reference_trajectory::AnalyticReference ReferenceActivationOutputConsumer::makeActivationMessage(
    const ::state_machine::Event& event, const SensorData& sensor, const ControllerConfig& config) {
    ...
    msg.analytic_type = reference_trajectory::AnalyticReference::ANALYTIC_CIRCLE_ENTRY;
    msg.flags = 0U;
