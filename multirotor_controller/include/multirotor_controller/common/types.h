...
enum class ControlMode {
  PX4_CASCADE_PID =
      0, // PX4串级PID：发送 pos+vel+acc（前馈），type_mask=0b111111000111
  PURE_SLIDING_MODE =
      1, // 纯滑模控制：只发送 acc=u_h+u_f，type_mask=0b011111000111
  HYBRID_CONTROL =
      2 // 混合控制：发送 pos+vel+acc（滑模），type_mask=0b111111000111
};
...
struct ControllerConfig {
  ...
  bool skip_takeoff_init_disarm{
      false}; // 是否跳过起飞初始化阶段的 DISARM 与 ALTCTL 安全门
  ...
  struct UavNmpcParams {
    ...
    double reference_sample_dt{0.01};
...
constexpr uint32_t ALTCTL_READY =
    16; // 定高模式就绪（接收到足够的ALTCTL状态帧）
constexpr uint32_t OFFBOARD_READY =
    17; // OFFBOARD模式就绪（接收到足够的OFFBOARD状态帧）
...
struct MpcTrajectoryState {
  ...
  bool new_data_received{
      false}; // 保留字段：MPC帧切换由 active/pending 缓存控制
