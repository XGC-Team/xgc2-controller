...
inline bool sendArmDisarmCommand(ros::ServiceClient *client, bool arm,
                                 bool force_disarm, const char *log_prefix) {
  if (!client) {
  ...
  if (client->call(srv)) {
    const char *action =
        arm ? "Arm" : (force_disarm ? "ForceDisarm" : "Disarm");
...
public:
  ...
  std::string name() const override { return arm_ ? "Arm" : "Disarm"; }
...
public:
  ...
  void execute(ros::NodeHandle &nh) override {
    ...
    if (client_->call(srv)) {
      ROS_INFO("[SetModeOutputTask] Set mode to '%s', result: %s",
               mode_.c_str(), srv.response.mode_sent ? "SUCCESS" : "FAILED");
    } else {
      ROS_ERROR("[SetModeOutputTask] Failed to call set_mode service");
    }
  }

  std::string name() const override { return "SetMode(" + mode_ + ")"; }
