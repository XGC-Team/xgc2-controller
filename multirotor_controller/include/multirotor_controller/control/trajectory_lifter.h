...
public:
    ...
    inline Setpoint lift(const MpcTrajectoryState& mpc_traj,
        ...
        if (dt < 0.0) {
            ROS_WARN_THROTTLE(1.0, "[TrajectoryLifter] Negative time offset: %.3f s, clock issue?", dt);
            dt = 0.0;
        }
        ...
        const Eigen::Vector3d pos_t = mpc_traj.position_k +
                                      mpc_traj.velocity_k * dt +
                                      0.5 * mpc_traj.acceleration_k * dt * dt;
        const Eigen::Vector3d vel_t = mpc_traj.velocity_k +
                                      mpc_traj.acceleration_k * dt;
    ...
    inline const TrajectoryLifterConfig& config() const { return config_; }
