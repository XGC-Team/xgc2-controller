...
#include "multirotor_controller/control/sliding_mode_controller.h"
#include "multirotor_controller/control/trajectory_lifter.h"
#include "multirotor_controller/state_machine/timing.h"
...
public:
  ...
  std::string name() const override { return "Custom1"; }

protected:
  ::state_machine::ActionResult
  onEnter(::state_machine::StateContext &ctx) override;
  ::state_machine::ActionResult
  onTick(::state_machine::StateContext &ctx) override;
  ::state_machine::ActionResult
  onExit(::state_machine::StateContext &ctx) override;
...
private:
  ...
  // ========== 数据字段：参考轨迹和控制输出分离 ==========
  // 参考轨迹（MPC插值结果，只读，不修改）
  Setpoint reference_trajectory_;
  ...
  // ========== 滑模控制器（用于模式1和模式2）==========
  SlidingModeController sliding_controller_{
      3.0, 3.0, 0.5}; // 默认参数：k1=3.0, k2=3.0, epsilon=0.5
  ...
  // ========== 辅助函数 ==========
  void handleNmpcEventMode(::state_machine::StateContext &ctx,
                           double current_time);
  void consumeNmpcResult(::state_machine::StateContext &ctx,
                         double current_time);
  void dispatchNmpcRequest(::state_machine::StateContext &ctx,
                           double current_time);
  void publishBackupSetpoint(::state_machine::StateContext &ctx,
                             double current_time);
  ...
  /// @brief 计算位置和速度跟踪误差
  void computeTrackingErrors(const SensorData &sensor_data,
                             Eigen::Vector3d &e_p, Eigen::Vector3d &e_v) const;
