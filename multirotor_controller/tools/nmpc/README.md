...
`multirotor_controller` C++ UAV NMPC backend. It lives inside the C++ controller
...
```bash
PYTHONPATH=source/ros1_ws/src/controller/multirotor_controller/tools \
PYTHONPYCACHEPREFIX="${PWD}/devel/.cache/multirotor_controller/pycache" \
...
```bash
PYTHONPATH=source/ros1_ws/src/controller/multirotor_controller/tools \
MPLCONFIGDIR="${PWD}/devel/.cache/multirotor_controller/matplotlib" \
PYTHONPYCACHEPREFIX="${PWD}/devel/.cache/multirotor_controller/pycache" \
python3 -B -m nmpc.runner --task all --tsim 20.0 \
  --dt-ctrl 0.01 --dt-sim 0.005 --mpc-div 10 --mpc-horizon 1.0 \
  --max-iter 20 --plot \
  --output-dir source/ros1_ws/src/controller/multirotor_controller/tools/nmpc/results/current_commit_validation
```
...
python3 -B -m nmpc.runner --task hover --tsim 20.0 \
  ...
  --output-dir source/ros1_ws/src/controller/multirotor_controller/tools/nmpc/results/perturbed_hover_20s
```
...
python3 -B -m nmpc.runner --task all --tsim 20.0 \
  ...
  --output-dir source/ros1_ws/src/controller/multirotor_controller/tools/nmpc/results/perturbed_all_wind_20s
```
...
```text
source/ros1_ws/src/controller/multirotor_controller/tools/nmpc/results/latest/
```
...
python3 -B -m unittest discover \
  -s source/ros1_ws/src/controller/multirotor_controller/tools/nmpc/tests -v
```
...
`source/ros1_ws/src/controller/multirotor_controller/generated/nmpc/uav_nmpc/`.
That directory is ignored by git and is compiled by `multirotor_controller` as an
