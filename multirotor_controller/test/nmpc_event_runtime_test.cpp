...
reference_trajectory::AnalyticReference makeAnalyticReference() {
    reference_trajectory::AnalyticReference msg;
    msg.header.stamp = ros::Time(10.0);
    msg.request_id = 1U;
    msg.trajectory_id = 2U;
    msg.revision = 3U;
    msg.analytic_type = reference_trajectory::AnalyticReference::ANALYTIC_HEIGHT_CIRCLE;
...
reference_trajectory::SampledReference makeSampledReference() {
    reference_trajectory::SampledReference msg;
    ...
    msg.sample_dt = 0.1;
    for (int i = 0; i < 3; ++i) {
        reference_trajectory::FlatReferencePoint point;
...
TEST(SensorChecks, StateEstimateGateIsOnlyControlStateSource) {
    ...
    sensor.uav_state_estimator_state = estimator_vrpn_px4_rotor_state::RigidStateEstimate::STATE_RUNNING;
    sensor.uav_state_estimator_flags = 0u;
    EXPECT_TRUE(sensor_checks::isStateEstimateUsableForControl(sensor));

    sensor.uav_state_estimator_state = estimator_vrpn_px4_rotor_state::RigidStateEstimate::STATE_COASTING;
    EXPECT_TRUE(sensor_checks::isStateEstimateUsableForControl(sensor));

    sensor.uav_state_estimator_flags = estimator_vrpn_px4_rotor_state::RigidStateEstimate::FLAG_FAULT;
    EXPECT_FALSE(sensor_checks::isStateEstimateUsableForControl(sensor));

    sensor.uav_state_estimator_flags = 0u;
    sensor.uav_state_estimator_state = estimator_vrpn_px4_rotor_state::RigidStateEstimate::STATE_SELF_CHECK;
    EXPECT_FALSE(sensor_checks::isStateEstimateUsableForControl(sensor));
}
